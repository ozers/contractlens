...
- Zero extra dependencies — only ajv, ajv-formats, swagger-parser
...
**Key modules:**
- `src/core/spec-loader.ts` — Parses OpenAPI specs via swagger-parser, caches by path. Builds a `Map<"METHOD /path", RouteSchema>` lookup.
